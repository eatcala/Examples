    ...
    if (P2L_Buffer[PositionLastData] == '\r')
    {
        ...
        for (uint8_t i = 0; i < PIPE_TO_LUOS_MAX_TASK; i++)
        {
        	if(P2LTask[i].data_pt == 0)
        	{
				...
				if(PositionLastData < PIPE_TO_LUOS_BUFFER_SIZE -1)
				{
					P2LBuffer_PrevStartData = PositionLastData + 1;
				}
				else
				{
					P2LBuffer_PrevStartData = 0;
...
static uint8_t PipeBuffer_P2LTaskNeedClear(uint16_t PositionLastData)
{
	...
	if(P2LTaskID > 0)
	{
		if(((&P2L_Buffer[PositionLastData]>(P2LTask[0].data_pt))&&(&P2L_Buffer[PositionLastData]<(P2LTask[0].end)))||
		((&P2L_Buffer[PositionLastData]<(P2LTask[0].data_pt))&&(&P2L_Buffer[PositionLastData]>(P2LTask[0].end))))
		{
			result = true;
