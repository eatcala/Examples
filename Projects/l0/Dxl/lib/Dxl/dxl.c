...
profile_core_t dxl_motor_profile[MAX_SERVICE_NUMBER];
...
static void discover_dxl(void)
{
    ...
    for (int i = 0; i < MAX_ID; i++)
    {
        ...
        if (!error)
        {
            ...
            // ************** Service creation *****************
            Luos_LinkServoMotorProfile(&dxl_motor_profile[y], &dxl[i].dxl_motor, Dxl_MsgHandler);
            Luos_LaunchProfile(&dxl_motor_profile[y], alias, revision);
...
static int find_id(service_t *service)
{
    ...
    {
        if ((int)service->profile_context == (int)&dxl_motor_profile[i])
            return i;
    }
    return i;
}
